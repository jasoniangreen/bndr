...
function _prepareAndCreateComponent(compEl, compOptions, attrName, CompClass) {
    ...
    if (!comp) 
        return console.error('bndr: component class ' + CompClass + ' did not return an instance.');

    if (bndr._components[compName])
        return logger.warn('bndr: component with name ' + compName + ' already created');

    // Keep a reference to the component/s on the element, must be an array to support multiples
    if (!compEl[bndr.COMPONENT_PROPERTY]) compEl[bndr.COMPONENT_PROPERTY] = [];
    compEl[bndr.COMPONENT_PROPERTY].push(comp);

    compEl.setAttribute(attrName, compName);
...
/**
 * Get components by element or by name
 * 
 * @param  {HTMLElement|String} elOrName An element
 * @param  {Boolean}            lookUp   `true` to search up dom tree for component, `false` by default
 * @return {Array|Object}                Array of component instances or single component
 */
bndr.getComponents = function getComponent(elOrName, lookUp) {
    if (typeof elOrName == 'string') {
        var comp = bndr._components[elOrName]
    } else {
        comp = elOrName && elOrName[bndr.COMPONENT_PROPERTY];
        do {
            comp = elOrName && elOrName[bndr.COMPONENT_PROPERTY];
            if (!comp) elOrName = elOrName.parentNode;
        } while (lookUp && !comp && elOrName);
    }

    return Array.isArray(comp) && comp.length === 1 ? comp[0] : comp;
}
